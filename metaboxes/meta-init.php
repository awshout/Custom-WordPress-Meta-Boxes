 ...
 */
define( 'CUSTOM_METABOXES_DIR', get_template_directory_uri() . '/library/includes/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	$size = isset( $field['size'] ) ? $field['size'] . 'em' : '25em';
					...
					switch( $type ) {
						...
						case 'select':
							echo '<select name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '"' , isset( $multiple ) && $multiple == true ? ' multiple="multiple"' : '' , '>
									<option value="">&mdash; ' . __('Select') . ' &mdash;</option>'; // Select One
...
class Custom_Add_Meta_Box {
    ...
    public function __construct( $id, $title, $page, $show_on=null, $fields ) {
		$this->id = $id;
		$this->title = $title;
		$this->page = $page;
		...
		add_action( 'admin_head', array( $this, 'show_for_attr' ) );

		add_action( 'add_meta_boxes', array( $this, 'add_meta_box' ) );
		add_action( 'save_post',  array( $this, 'save_meta_box' ));
    }
	
	// Show or hide based on page template using JavaScript
	function show_for_attr() {
		$output = '';
		if ( $this->show_on ) {
			$output .= 'jQuery(function($) {';
			$output .= '$("#' . $this->id . '").attr("data-metabox-template", "' . $this->show_on . '")';
			$output .= '});';
		}
		...
		foreach ( $this->fields as $field) {
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr>
						...
						$meta_array = get_post_meta( get_the_ID(), $this->id, true );
						$meta = ( !empty( $meta_array ) && isset( $meta_array[$field['id']] ) ) ? $meta_array[$field['id']] : null;
	...
	function save_meta_box( $post_id ) {
		...
		// verify nonce
		if ( !isset( $_POST['meta_box_nonce_field'] ) )
			return $post_id;
		if ( !( in_array( $post_type, $this->page ) || wp_verify_nonce( $_POST['meta_box_nonce_field'],  'meta_box_nonce_action' ) ) )
			return $post_id;
		...
		foreach ( $this->fields as $field ) {
			...
			if( $field['type'] == 'editor' && empty($sanitizer) ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				wp_set_object_terms( $post_id, $term, $field['taxonomy'] );
			}
			...
			if ( isset( $new ) && $new != $old ) {
				$sanitizer = isset( $field['sanitizer'] ) ? $field['sanitizer'] : 'none';
				if ( is_array( $new ) ) {
			...
			$new_meta_key[] = $field['id'];
			$new_meta_value[] = $new;

			$new_meta_array = array_combine($new_meta_key, $new_meta_value);
		}
	
		// If a new meta value was added and there was no previous value, add it
		if ( $new_meta_array && '' == $meta_value )
			add_post_meta( $post_id, $meta_key, $new_meta_array, true );
	
		// If the new meta value does not match the old value, update it
		elseif ( $new_meta_array && $new_meta_array != $meta_value )
			update_post_meta( $post_id, $meta_key, $new_meta_array );
	
		// If there is no new meta value but an old value exists, delete it
		elseif ( '' == $new_meta_array && $meta_value )
